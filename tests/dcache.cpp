    ...
    if(!filename1.empty())
                 {
         	        char *cstr = new char[filename1.length() + 1];
              	    strcpy(cstr, filename1.c_str());
                   fprintf(traceins,"W,0x%x,#%s,%d,%d,%x\n",(ADDRINT)ip,cstr,line, column, addr);
                 }
...
int main(int argc, char* argv[]) {
    ...
    char name3[100] = "pinatrace_suc.out.";
    char var[10];
    sprintf(var, "%d", lb);
    tracemem = fopen(strcat(strcat(name1, var), ".txt"), "w");
    traceins = fopen(strcat(strcat(name2, var), ".txt"), "w");
    tracesuc = fopen(strcat(strcat(name3, var), ".txt"), "w");
    fprintf(tracemem, "SafeCopyAddress\n");
    fprintf(traceins, "R/W,InsAddress,FileName,Line,Column,EA\n");
